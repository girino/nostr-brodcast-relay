...
func (d *Discovery) fetchRelaysFromRelay(ctx context.Context, relayURL string) []string {
	...
	if err != nil {
		log.Printf("Failed to connect to seed relay %s: %v", relayURL, err)
		return []string{}
	}
	defer relay.Close()
	...
	log.Printf("Fetched %d relay URLs from %s", len(result), relayURL)
	return result
}
...
func (d *Discovery) isAlreadyKnown(url string) bool {
	return d.manager.GetRelayInfo(url) != nil
}
